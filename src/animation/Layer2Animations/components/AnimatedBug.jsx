import React from 'react';

function AnimatedBug({ className = '', style = {} }) {
  return (
    <>
      <style>{`
        .animated-bug-container {
          position: relative;
          user-select: none;
          pointer-events: none;
        }

        .bug-leg-left-1, .bug-leg-left-2, .bug-leg-left-3,
        .bug-leg-right-1, .bug-leg-right-2, .bug-leg-right-3 {
          transform-box: fill-box;
          animation-duration: 0.45s;
          animation-iteration-count: infinite;
          animation-timing-function: ease-in-out;
        }

        .bug-leg-left-1 { transform-origin: 90% 90%; }
        .bug-leg-left-2 { transform-origin: 90% 50%; }
        .bug-leg-left-3 { transform-origin: 90% 10%; }

        .bug-leg-right-1 { transform-origin: 10% 90%; }
        .bug-leg-right-2 { transform-origin: 10% 50%; }
        .bug-leg-right-3 { transform-origin: 10% 10%; }

        .bug-leg-left-1, .bug-leg-right-2, .bug-leg-left-3 {
          animation-name: bugLegGroupA;
        }

        .bug-leg-right-1, .bug-leg-left-2, .bug-leg-right-3 {
          animation-name: bugLegGroupB;
        }

        @keyframes bugLegGroupA {
          0%, 100% { transform: rotate(-6deg); }
          50% { transform: rotate(6deg); }
        }

        @keyframes bugLegGroupB {
          0%, 100% { transform: rotate(6deg); }
          50% { transform: rotate(-6deg); }
        }

        .bug-antenna {
          transform-box: fill-box;
          animation-duration: 1.3s;
          animation-iteration-count: infinite;
          animation-timing-function: ease-in-out;
        }

        .bug-antenna-left {
          transform-origin: 80% 90%;
          animation-name: bugAntennaLeft;
        }

        .bug-antenna-right {
          transform-origin: 20% 90%;
          animation-name: bugAntennaRight;
        }

        @keyframes bugAntennaLeft {
          0%, 100% { transform: rotate(0deg); }
          50% { transform: rotate(-5deg); }
        }

        @keyframes bugAntennaRight {
          0%, 100% { transform: rotate(0deg); }
          50% { transform: rotate(5deg); }
        }
      `}</style>
      <div className={`animated-bug-container ${className}`} style={{ width: 60, height: 60, display: 'inline-block', ...style }}>
        <svg
          viewBox="0 0 512.004 512.004"
          width="100%"
          height="100%"
          xmlSpace="preserve"
        >
          <g transform="translate(0 -1)">
            {/* ANTENNAE */}
            <g className="bug-antennae">
              {/* Left Antenna */}
              <path
                className="bug-antenna bug-antenna-left"
                style={{ fill: '#95A5A5' }}
                d="M229.96,79.102c-4.487,0-8.287-3.454-8.643-8.01c-0.043-0.521-4.487-52.736-34.738-52.736
                  c-4.79,0-8.678-3.879-8.678-8.678c0-4.79,3.888-8.678,8.678-8.678c46.288,0,51.833,65.961,52.042,68.773
                  c0.364,4.773-3.211,8.938-7.984,9.303C230.411,79.093,230.185,79.102,229.96,79.102"
              />
              {/* Right Antenna */}
              <path
                className="bug-antenna bug-antenna-right"
                style={{ fill: '#95A5A5' }}
                d="M282.045,79.106c-0.217,0-0.443-0.009-0.668-0.026c-4.773-0.356-8.357-4.53-7.992-9.311
                  c0.208-2.803,5.753-68.764,52.042-68.764c4.79,0,8.678,3.888,8.678,8.678s-3.888,8.678-8.678,8.678
                  c-30.382,0-34.695,52.198-34.738,52.727C290.35,75.644,286.54,79.106,282.045,79.106"
              />
            </g>

            {/* LEGS */}
            <g className="bug-legs">
              {/* Left Front Leg (bug-leg-left-1) */}
              <path
                className="bug-leg bug-leg-left-1"
                style={{ fill: '#95A5A5' }}
                d="M160.54,183.246c-1.137,0-2.3-0.226-3.419-0.712L96.375,156.5
                  c-3.185-1.362-5.259-4.504-5.259-7.966V53.076H39.049c-4.79,0-8.678-3.888-8.678-8.678c0-4.799,3.888-8.678,8.678-8.678h60.746
                  c4.799,0,8.678,3.879,8.678,8.678v98.408l55.496,23.786c4.4,1.883,6.439,6.986,4.547,11.394
                  C167.109,181.267,163.907,183.246,160.54,183.246"
              />
              {/* Right Front Leg (bug-leg-right-1) */}
              <path
                className="bug-leg bug-leg-right-1"
                style={{ fill: '#95A5A5' }}
                d="M351.464,183.246c-3.367,0-6.569-1.979-7.975-5.259c-1.892-4.408,0.147-9.511,4.547-11.394
                  l55.496-23.786V44.399c0-4.799,3.879-8.678,8.678-8.678h60.746c4.79,0,8.678,3.879,8.678,8.678c0,4.79-3.888,8.678-8.678,8.678
                  h-52.068v95.458c0,3.463-2.074,6.604-5.259,7.966l-60.746,26.034C353.764,183.02,352.601,183.246,351.464,183.246"
              />
              {/* Left Middle Leg (bug-leg-left-2) */}
              <path
                className="bug-leg bug-leg-left-2"
                style={{ fill: '#95A5A5' }}
                d="M91.121,252.665c-1.501,0-2.985-0.391-4.304-1.145l-60.746-34.712
                  c-4.165-2.378-5.606-7.68-3.228-11.837s7.689-5.597,11.837-3.228l57.925,33.098l40.205-8.036
                  c4.712-0.946,9.277,2.109,10.205,6.804c0.946,4.695-2.1,9.268-6.804,10.205l-43.39,8.678
                  C92.258,252.613,91.685,252.665,91.121,252.665"
              />
              {/* Right Middle Leg (bug-leg-right-2) */}
              <path
                className="bug-leg bug-leg-right-2"
                style={{ fill: '#95A5A5' }}
                d="M420.884,252.665c-0.564,0-1.137-0.052-1.701-0.174l-43.39-8.678
                  c-4.703-0.937-7.749-5.511-6.804-10.205c0.929-4.695,5.493-7.749,10.205-6.804l40.205,8.036l57.925-33.098
                  c4.157-2.369,9.468-0.929,11.837,3.228c2.378,4.157,0.937,9.459-3.228,11.837l-60.746,34.712
                  C423.869,252.275,422.385,252.665,420.884,252.665"
              />
              {/* Left Hind Leg (bug-leg-left-3) */}
              <path
                className="bug-leg bug-leg-left-3"
                style={{ fill: '#95A5A5' }}
                d="M91.112,513.004c-1.458,0-2.942-0.364-4.296-1.145c-4.165-2.378-5.606-7.68-3.228-11.837
                  l32.508-56.893l-41.55-91.414c-1.623-3.558-0.66-7.758,2.343-10.257l52.068-43.39c3.679-3.072,9.147-2.577,12.219,1.111
                  c3.072,3.679,2.569,9.155-1.111,12.219l-46.983,39.155l40.648,89.435c1.154,2.543,1.024,5.476-0.364,7.897l-34.712,60.746
                  C97.057,511.434,94.123,513.004,91.112,513.004"
              />
              {/* Right Hind Leg (bug-leg-right-3) */}
              <path
                className="bug-leg bug-leg-right-3"
                style={{ fill: '#95A5A5' }}
                d="M420.892,513.004c-3.011,0-5.944-1.571-7.541-4.374l-34.712-60.746
                  c-1.388-2.421-1.519-5.354-0.364-7.897l40.648-89.435l-46.982-39.155c-3.679-3.063-4.183-8.539-1.111-12.219
                  c3.063-3.679,8.539-4.174,12.219-1.111l52.068,43.39c3.003,2.499,3.966,6.699,2.343,10.257l-41.55,91.414l32.508,56.893
                  c2.378,4.157,0.937,9.459-3.228,11.837C423.834,512.64,422.35,513.004,420.892,513.004"
              />
            </g>

            {/* HEAD & BODY */}
            <path
              style={{ fill: '#4C8056' }}
              d="M256.002,87.784c12.019,0,23.474,2.317,34.07,6.37c0.391-2.065,0.642-4.191,0.642-6.37
                c0-19.17-15.542-34.712-34.712-34.712S221.29,68.614,221.29,87.784c0,2.178,0.252,4.304,0.642,6.37
                C232.528,90.101,243.983,87.784,256.002,87.784"
            />
            <path
              style={{ fill: '#4C8056' }}
              d="M256.002,486.97L256.002,486.97c-67.098,0-121.492-54.393-121.492-121.492V209.276h242.983v156.203
                C377.494,432.577,323.1,486.97,256.002,486.97"
            />
            <g>
              <path
                style={{ fill: '#396640' }}
                d="M351.46,209.276H160.545v-26.034c0-52.719,42.739-95.458,95.458-95.458
                  s95.458,42.739,95.458,95.458V209.276z"
              />
              <path style={{ fill: '#396640' }} d="M342.782,450.428V209.276h-17.356v255.826C331.665,460.746,337.48,455.843,342.782,450.428" />
              <path style={{ fill: '#396640' }} d="M290.714,481.873V209.272h-17.356v276.315C279.285,484.745,285.099,483.548,290.714,481.873" />
              <path style={{ fill: '#396640' }} d="M238.646,485.589V209.274H221.29v272.601C226.905,483.55,232.719,484.738,238.646,485.589" />
              <path style={{ fill: '#396640' }} d="M186.578,465.102V209.276h-17.356v241.152C174.525,455.843,180.339,460.754,186.578,465.102" />
            </g>
          </g>
        </svg>
      </div>
    </>
  );
}

export default AnimatedBug;
