...
export function BombSequence({
  ...
  // ── Impact position (measured from lineContainerRef, updates on resize) ───────────
  const impactPos = useImpactPosition(lineContainerRef);

  // ── FLIP animation: recordFirstPositions must fire BEFORE setLines ──────────
  const { recordFirstPositions } = useFlipJumble({
    lineContainerRef,
    lines: lines || finalLines,
  });
  ...
  const wrappedOnJumble = React.useCallback(() => {
    // Stop shaking instantly so FLIP can measure true positions and apply inline transforms
    if (codeBoxRef?.current) {
      codeBoxRef.current.classList.remove('anim-impact-shake');
    }
    if (lineContainerRef?.current) {
      const items = lineContainerRef.current.querySelectorAll('.anim-target-line');
      items.forEach(el => el.classList.remove('anim-line-impact-shake'));
    }
  ...
  React.useEffect(() => {
    ...
    if (animPhase === 'impact') {
      ...
      if (lineList) {
        const items = lineList.querySelectorAll('.anim-target-line');
        items.forEach((el) => {
