import React from 'react';

function AnimatedAnt({ className = '', style = {} }) {
  return (
    <>
      <style>{`
        .animated-ant-container {
          position: relative;
          user-select: none;
          pointer-events: none;
        }

        .ant-leg-left-1, .ant-leg-left-2, .ant-leg-left-3,
        .ant-leg-right-1, .ant-leg-right-2, .ant-leg-right-3 {
          transform-box: fill-box;
          animation-duration: 0.4s;
          animation-iteration-count: infinite;
          animation-timing-function: ease-in-out;
        }

        .ant-leg-left-1 { transform-origin: 90% 90%; }
        .ant-leg-left-2 { transform-origin: 90% 50%; }
        .ant-leg-left-3 { transform-origin: 90% 10%; }

        .ant-leg-right-1 { transform-origin: 10% 90%; }
        .ant-leg-right-2 { transform-origin: 10% 50%; }
        .ant-leg-right-3 { transform-origin: 10% 10%; }

        .ant-leg-left-1, .ant-leg-right-2, .ant-leg-left-3 {
          animation-name: antLegGroupA;
        }

        .ant-leg-right-1, .ant-leg-left-2, .ant-leg-right-3 {
          animation-name: antLegGroupB;
        }

        @keyframes antLegGroupA {
          0%, 100% { transform: rotate(-7deg); }
          50% { transform: rotate(7deg); }
        }

        @keyframes antLegGroupB {
          0%, 100% { transform: rotate(7deg); }
          50% { transform: rotate(-7deg); }
        }

        .ant-antenna {
          transform-box: fill-box;
          animation-duration: 1.2s;
          animation-iteration-count: infinite;
          animation-timing-function: ease-in-out;
        }

        .ant-antenna-left {
          transform-origin: 80% 90%;
          animation-name: antAntennaLeft;
        }

        .ant-antenna-right {
          transform-origin: 20% 90%;
          animation-name: antAntennaRight;
        }

        @keyframes antAntennaLeft {
          0%, 100% { transform: rotate(0deg); }
          50% { transform: rotate(-6deg); }
        }

        @keyframes antAntennaRight {
          0%, 100% { transform: rotate(0deg); }
          50% { transform: rotate(6deg); }
        }
      `}</style>
      <div className={`animated-ant-container ${className}`} style={{ width: 60, height: 60, display: 'inline-block', ...style }}>
        <svg
          viewBox="0 0 512.005 512.005"
          width="100%"
          height="100%"
          xmlSpace="preserve"
        >
          <g transform="translate(0 -1)">
            {/* LEGS & ANTENNAE GROUP */}
            <g className="ant-limbs">
              {/* Left Hind Leg (ant-leg-left-3) */}
              <path
                className="ant-leg ant-leg-left-3"
                style={{ fill: '#472914' }}
                d="M85.336,513.005c-2.427,0-4.855-0.929-6.703-2.778c-3.707-3.707-3.707-9.7,0-13.407l26.368-26.368
                  l27.913-148.821c0.474-2.588,2.001-4.845,4.219-6.258l104.296-66.37c4.437-2.807,10.287-1.498,13.094,2.911
                  c2.816,4.418,1.508,10.278-2.911,13.094l-100.826,64.161l-27.695,147.655c-0.351,1.877-1.252,3.603-2.607,4.959l-28.444,28.444
                  C90.191,512.076,87.763,513.005,85.336,513.005"
              />
              {/* Left Middle Leg (ant-leg-left-2) */}
              <path
                className="ant-leg ant-leg-left-2"
                style={{ fill: '#472914' }}
                d="M104.299,257.005H66.373c-5.243,0-9.481-4.248-9.481-9.481c0-5.234,4.238-9.481,9.481-9.481h35.3
                  l45.151-27.089c1.479-0.891,3.157-1.356,4.883-1.356h66.37c5.243,0,9.481,4.248,9.481,9.481s-4.238,9.481-9.481,9.481h-63.744
                  l-45.151,27.089C107.703,256.541,106.025,257.005,104.299,257.005"
              />
              {/* Left Front Leg (ant-leg-left-1) */}
              <path
                className="ant-leg ant-leg-left-1"
                style={{ fill: '#472914' }}
                d="M218.086,190.635c-0.616,0-1.242-0.057-1.868-0.19l-47.407-9.481
                  c-2.465-0.484-4.636-1.944-6.03-4.03l-37.926-56.889c-1.033-1.555-1.593-3.394-1.593-5.262V41.804l-17.37-26.055
                  c-2.911-4.361-1.735-10.249,2.626-13.151c4.361-2.911,10.24-1.735,13.151,2.626l18.963,28.444
                  c1.033,1.555,1.593,3.394,1.593,5.262v72.979l34.162,51.238l43.548,8.713c5.129,1.024,8.467,6.021,7.433,11.15
                  C226.477,187.516,222.514,190.635,218.086,190.635"
              />
              {/* Right Hind Leg (ant-leg-right-3) */}
              <path
                className="ant-leg ant-leg-right-3"
                style={{ fill: '#472914' }}
                d="M426.669,513.005c-2.427,0-4.855-0.929-6.703-2.778l-28.444-28.444
                  c-1.356-1.356-2.257-3.082-2.607-4.959l-27.695-147.655l-100.826-64.161c-4.418-2.816-5.727-8.676-2.911-13.094
                  c2.816-4.409,8.676-5.717,13.094-2.911l104.296,66.37c2.219,1.413,3.745,3.669,4.219,6.258l27.913,148.821l26.368,26.368
                  c3.707,3.707,3.707,9.7,0,13.407C431.524,512.076,429.097,513.005,426.669,513.005"
              />
              {/* Right Middle Leg (ant-leg-right-2) */}
              <path
                className="ant-leg ant-leg-right-2"
                style={{ fill: '#472914' }}
                d="M445.632,257.005h-37.926c-1.726,0-3.404-0.465-4.883-1.356l-45.151-27.089h-63.744
                  c-5.243,0-9.481-4.248-9.481-9.481s4.238-9.481,9.481-9.481h66.37c1.726,0,3.404,0.465,4.883,1.356l45.151,27.089h35.3
                  c5.243,0,9.481,4.248,9.481,9.481C455.114,252.758,450.876,257.005,445.632,257.005"
              />
              {/* Right Front Leg (ant-leg-right-1) */}
              <path
                className="ant-leg ant-leg-right-1"
                style={{ fill: '#472914' }}
                d="M293.919,190.635c-4.428,0-8.391-3.119-9.282-7.623c-1.033-5.129,2.304-10.126,7.434-11.15
                  l43.548-8.713l34.162-51.238V38.931c0-1.868,0.559-3.707,1.593-5.262l18.963-28.444c2.901-4.361,8.77-5.537,13.151-2.626
                  c4.361,2.901,5.537,8.789,2.626,13.151l-17.37,26.055v72.979c0,1.868-0.559,3.707-1.593,5.262l-37.926,56.889
                  c-1.394,2.086-3.565,3.546-6.03,4.03l-47.407,9.481C295.161,190.578,294.526,190.635,293.919,190.635"
              />
            </g>

            {/* ANTENNAE */}
            <g className="ant-antennae">
              {/* Left Antenna */}
              <path
                className="ant-antenna ant-antenna-left"
                style={{ fill: '#BDC3C7' }}
                d="M227.558,76.857c-20.916,0-37.926-17.01-37.926-37.926s17.01-37.926,37.926-37.926
                  c5.243,0,9.481,4.248,9.481,9.481s-4.238,9.481-9.481,9.481c-10.458,0-18.963,8.505-18.963,18.963s8.505,18.963,18.963,18.963
                  c5.243,0,9.481,4.248,9.481,9.481C237.04,72.61,232.802,76.857,227.558,76.857"
              />
              {/* Right Antenna */}
              <path
                className="ant-antenna ant-antenna-right"
                style={{ fill: '#BDC3C7' }}
                d="M284.447,76.857c-5.243,0-9.482-4.248-9.482-9.481c0-5.234,4.238-9.481,9.482-9.481
                  c10.458,0,18.963-8.505,18.963-18.963s-8.505-18.963-18.963-18.963c-5.243,0-9.482-4.248-9.482-9.481s4.238-9.481,9.482-9.481
                  c20.916,0,37.926,17.01,37.926,37.926S305.363,76.857,284.447,76.857"
              />
            </g>

            {/* HEAD & THORAX */}
            <g className="ant-head-thorax">
              <polygon style={{ fill: '#633C21' }} points="237.04,323.376 274.966,323.376 274.966,228.561 237.04,228.561" />
              <path
                style={{ fill: '#633C21' }}
                d="M266.281,162.191h-20.556c-25.742,0-46.611-20.869-46.611-46.62V95.033
                  c0-25.752,20.869-46.62,46.611-46.62h20.556c25.742,0,46.611,20.869,46.611,46.62v20.537
                  C312.892,141.322,292.023,162.191,266.281,162.191"
              />
            </g>

            {/* ABDOMEN */}
            <g className="ant-abdomen">
              <path
                style={{ fill: '#805333' }}
                d="M303.41,200.117c0,20.945-21.22,37.926-47.407,37.926s-47.407-16.981-47.407-37.926
                  s21.22-37.926,47.407-37.926S303.41,179.172,303.41,200.117"
              />
              <path
                style={{ fill: '#805333' }}
                d="M256.003,475.079L256.003,475.079c-36.655,0-66.37-29.715-66.37-66.37v-37.926
                  c0-36.655,29.715-66.37,66.37-66.37s66.37,29.715,66.37,66.37v37.926C322.373,445.365,292.658,475.079,256.003,475.079"
              />
            </g>
          </g>
        </svg>
      </div>
    </>
  );
}

export default AnimatedAnt;
